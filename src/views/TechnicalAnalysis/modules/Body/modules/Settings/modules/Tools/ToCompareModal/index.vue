  ...
  <div class="toCompareModal">
    <div class="toCompareModal__container">
      <div class="toCompareModal__header">
        <div class="toCompareModal__title">Активы к сравнению</div>
        <svg
          fill="none"
          height="24"
          viewBox="0 0 24 24"
    ...
    <div v-if="isInit" class="toCompareModal__wrap">
      <div v-if="getAllAssets.length > 0" class="toCompareModal__items toCompareModal__container">
        <div v-for="asset in getAllAssets" :key="asset.ID" class="toCompareModal__item">
          {{ asset.NAME }}
          <svg
            v-if="isSelectedAsset(asset) === false"
            fill="none"
            height="34"
            viewBox="0 0 34 34"
            width="34"
            xmlns="http://www.w3.org/2000/svg"
            @click="onChangeSelect(asset)"
          >
...
export default defineComponent({
  name: 'TechnicalAnalysisPageToCompareModal',
  components: { Loader },
  emits: ['onClose'],
  setup(props, { emit }) {
    ...
    const onClickClose = () => {
      techStore.inputAssetSearch('');
      emit('onClose');
    };

    const onChangeSelect = async (asset: DynamicObject) => {
      await techStore.changeSelectedAssets(asset);
    };

    const isSelectedAsset = (asset: DynamicObject) => techStore.isSelectedAsset(asset);

    const onInputSearch = () => {
      techStore.inputAssetSearch(searchModel.value);
    };

    const init = async () => {
      if (techStore.allAssets.length > 0) {
        isInit.value = true;
      } else {
        await techStore.initAssets();
        isInit.value = true;
      }
    };
    ...
    return {
      ...
      isSelectedAsset,
      onInputSearch,
      getAllAssets: computed(() => techStore.allAssets),
      isInit,
      searchModel,
    };
  },
